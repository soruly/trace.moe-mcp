...
| :--------------------------- | :---------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `search_anime_by_image_url`  | Search anime scene by image URL (preprocessed locally into 33-element vector) | `url` (string, required)<br>`cutBorders` (boolean, default true)<br>`anilistInfo` (boolean, default true)<br>`anilistID` (number, optional)<br>`lang` ("en" \| "zh" \| "ja", optional)                                          |
| `search_anime_by_image_file` | Search anime scene from local file path or base64 string                      | `filePath` (string, optional)<br>`imageBase64` (string, optional)<br>`cutBorders` (boolean, default true)<br>`anilistInfo` (boolean, default true)<br>`anilistID` (number, optional)<br>`lang` ("en" \| "zh" \| "ja", optional) |
| `search_anime_by_vector`     | Search anime scene directly using a 33-element color layout vector            | `vector` (array of 33 numbers, required)<br>`anilistInfo` (boolean, default true)<br>`anilistID` (number, optional)<br>`lang` ("en" \| "zh" \| "ja", optional)                                                                  |
| `search_anime_by_name`       | Search anime metadata and retrieve Anilist IDs by name/title                  | `query` (string, required)<br>`lang` ("en" \| "zh" \| "ja", optional)                                                                                                                                                           |
| `get_account_quota`          | Check current daily search quota, concurrency limit, and priority             | `lang` ("en" \| "zh" \| "ja", optional)                                                                                                                                                                                         |
...
- `trace_moe` - Prompt template (available in English, Chinese, and Japanese) guiding the AI model on how to identify an anime screenshot.
...
- `TRACE_MOE_LANG`: Optional default language for anime titles and markdown output (`en`, `zh`, `ja`). Defaults to `en`.
...
Node.js (>= 22.6) executes TypeScript directly:
